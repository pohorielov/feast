...
export class AuthService {
  ...
  async registration(userData: CreateUserDto) {
    ...
    if (candidate) {
      throw new HttpException('Email вже зареєсрований', HttpStatus.BAD_REQUEST);
    }
  ...
  async generateTokens(user: User) {
    ...
    return {
      accessToken: await this.jwtService.sign(payload, { secret: process.env.ACCESS_SECRET_KEY, expiresIn: '15s' }),
      refreshToken: await this.jwtService.sign(payload, { secret: process.env.REFRESH_SECRET_KEY, expiresIn: '30s' }),
    };
  }
